...
class PluginSatisfactionReminder extends CommonDBTM {
    ...
    */
   static function getTicketSatisfaction($date_begin, $date_answered, $entities_id, $ticket_id = 0) {
      global $DB;
      ...
      $query .= (!is_null($date_begin) ? " AND ts.date_begin > DATE('" . $date_begin . "')" : '');
      $query .= " AND ts.date_answered " . (($date_answered == null) ? " IS NULL" : " = DATE('" . $date_answered . "')");
      $query .= ((int) $ticket_id > 0 ? " AND ts.ticket_id = " . $ticket_id : '');
   ...
   /**
    * Send Survey Satisfation for ticket
    *
    * @param TicketSatisfaction $ticketSatisfaction
    *
    * @return void
    */
   static function sendReminder(TicketSatisfaction $ticketSatisfaction) {
      ...
      foreach ($surveys as $survey) {
         ...
         // Send notification
         PluginSatisfactionNotificationTargetTicket::sendReminder($ticketSatisfaction->fields['tickets_id']);
      }
   }
